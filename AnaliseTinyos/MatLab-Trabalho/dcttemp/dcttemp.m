...
function varargout = dcttemp(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dcttemp_OpeningFcn, ...
                   'gui_OutputFcn',  @dcttemp_OutputFcn, ...
                   'gui_LayoutFcn',  @dcttemp_LayoutFcn, ...
                   'gui_Callback',   []);
...
function dcttemp_OpeningFcn(hObject, eventdata, handles, varargin)
handles.output = hObject;

guidata(hObject, handles);
axes(handles.axes9);
image(imread('../dcttemp/unb.png','png'));
axis off;
axes(handles.axes7);
image(imread('../dcttemp/grafo.png','png'));
axis off;
end


function varargout = dcttemp_OutputFcn(hObject, eventdata, handles) 
...
try
    ...
    if(handles.index2~=0)
        ...
        M=M';
        final=zeros(1,length(M));
        for i=1:length(M)
            final(1,i)=0.001010024 + 0.000242127*log(10000*(1023-M(1,i))/M(1,i)) + 0.000000146*power(log(10000*(1023-M(1,i))/M(1,i)),3);
        end
        T =power(final,-1);
        T=T-273.15;
        T=T';
        save('DCTTEMPERATURA.mat','T');
        dim=size(T);
        ...
        title('DADOS DE TEMPERATURA');%TITLE OF THE GRAPHIC
        xlabel('TEMPO (s)');
...
function h1 = dcttemp_LayoutFcn(policy)
...
'Callback',@(hObject,eventdata)dcttemp('bot_plot_Callback',hObject,eventdata,guidata(hObject)),...
'FontSize',14,...
...
'FontUnits','pixels',...
'BackgroundColor',[0.67843137254902 0.92156862745098 1],...
'Callback',@(hObject,eventdata)dcttemp('limp_Callback',hObject,eventdata,guidata(hObject)),...
'FontSize',14,...
...
h9 = uimenu(...
'Parent',h1,...
'Callback',@(hObject,eventdata)dcttemp('Arquivo_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Arquivo',...
'Tag','Arquivo',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
h10 = uimenu(...
'Parent',h9,...
'Callback',@(hObject,eventdata)dcttemp('Novo_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Novo',...
'Tag','Novo',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
h11 = uimenu(...
'Parent',h9,...
'Callback',@(hObject,eventdata)dcttemp('Sair_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Sair',...
'Tag','Sair',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
h12 = uimenu(...
'Parent',h1,...
'Callback',@(hObject,eventdata)dcttemp('Ajuda_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Ajuda',...
'Tag','Ajuda',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'FontUnits','pixels',...
'BackgroundColor',[0.67843137254902 0.92156862745098 1],...
'Callback',@(hObject,eventdata)dcttemp('audio_bot_Callback',hObject,eventdata,guidata(hObject)),...
'FontSize',14,...
'Position',[199.8 12.9230769230769 32.2 3.07692307692308],...
'String','Capturar Temperatura',...
'Tag','audio_bot',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
elseif local_isInvokeHGCallback(gui_State, varargin{:})
    % DCTTEMP('CALLBACK',hObject,eventData,handles,...)
    gui_Create = false;
else
    % DCTTEMP(...)
