...
M=textread('dadosdcttemp_temp.txt','%2c');
M=hex2dec(char(M));
M=reshape(M,n,[])';
dim=size(M);

M2=textread('dadosdcttemp_coef.txt','%2c');
M2=hex2dec(char(M2));
M2=reshape(M2,n2,[])';
dim2=size(M2);
...
bar_bits_temp = applyhatch_pluscolor(gcf,'|-',0,0);
imwrite(bar_bits_temp,'bar_bits_temp.png','png');