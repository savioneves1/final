...
axes(handles.axes9);
image(imread('../photo/unb.png','png'));
axis off;
axes(handles.axes7);
image(imread('../photo/grafo.png','png'));
axis off;
end
...
try
    ...
    if(handles.index2~=0)
        ...
        luz0=M(:,9);
        luz1=M(:,10);
        luz0=bitshift(luz0,8);
        luz=luz0+luz1;
        luz=luz.*2.64./1024;
        save('LUZ.mat','luz');
        handles.luz=luz;
        handles.sample=1:length(handles.luz);
        stem(handles.grafico2,handles.sample,handles.luz); % PLOTS AUDIO
        title('Tensão (V)');%TITLE OF THE GRAPHIC
        xlabel('TEMPO (s)');
        ylabel('LUZ');
        guidata(hObject, handles);%UPDATE THE ELEMENTS
...
function h1 = photo_LayoutFcn(policy)
...
'Callback',@(hObject,eventdata)photo('bot_plot_Callback',hObject,eventdata,guidata(hObject)),...
'FontSize',14,...
...
'FontUnits','pixels',...
'BackgroundColor',[0.67843137254902 0.92156862745098 1],...
'Callback',@(hObject,eventdata)photo('limp_Callback',hObject,eventdata,guidata(hObject)),...
'FontSize',14,...
...
h9 = uimenu(...
'Parent',h1,...
'Callback',@(hObject,eventdata)photo('Arquivo_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Arquivo',...
'Tag','Arquivo',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
h10 = uimenu(...
'Parent',h9,...
'Callback',@(hObject,eventdata)photo('Novo_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Novo',...
'Tag','Novo',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
h11 = uimenu(...
'Parent',h9,...
'Callback',@(hObject,eventdata)photo('Sair_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Sair',...
'Tag','Sair',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
h12 = uimenu(...
'Parent',h1,...
'Callback',@(hObject,eventdata)photo('Ajuda_Callback',hObject,eventdata,guidata(hObject)),...
'Label','Ajuda',...
'Tag','Ajuda',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'FontUnits','pixels',...
'BackgroundColor',[0.67843137254902 0.92156862745098 1],...
'Callback',@(hObject,eventdata)photo('audio_bot_Callback',hObject,eventdata,guidata(hObject)),...
'FontSize',14,...
...
elseif local_isInvokeHGCallback(gui_State, varargin{:})
    % PHOTO('CALLBACK',hObject,eventData,handles,...)
    gui_Create = false;
else
    % PHOTO(...)
